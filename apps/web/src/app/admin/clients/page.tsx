...
export default function AdminClientsPage() {
  const swrKey = "/api/admin/clients?includeInactive=true";
  const { data, error } = useSWR<{ items: Client[] }>(swrKey, () => apiFetch<{ items: Client[] }>(swrKey));
  ...
  async function onSubmit(values: ClientFormValues) {
    ...
    try {
      ...
      if (editingId) {
        await adminJson(`/api/admin/clients/${editingId}`, { method: "PUT", body: JSON.stringify(payload) });
        setNotification("Client updated.");
      } else {
        await adminJson(`/api/admin/clients`, { method: "POST", body: JSON.stringify(payload) });
      ...
      setNotification(err?.message ?? "Failed to save client.");
    }
  }
  ...
  async function uploadLogo(file: File) {
    ...
    try {
      ...
      setNotification(e?.message ?? "Logo upload failed.");
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <div className="text-[12px] font-bold tracking-wider text-slate-400 dark:text-[#94a3b8] uppercase">Clients</div>
        <h2 className="text-[20px] font-bold tracking-tight">Manage client showcase</h2>
      </div>
      ...
      {error ? <div className="text-sm text-red-500">Failed to load clients.</div> : null}

      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 lg:grid-cols-2">
        <div className="flex flex-col gap-3">
          <label className="flex flex-col gap-1">
            <span className="text-[11px] font-bold tracking-wide text-slate-500 dark:text-[#94a3b8] uppercase">Client name</span>
            <input
              {...register("name")}
          ...
          <label className="flex flex-col gap-1">
            <span className="text-[11px] font-bold tracking-wide text-slate-500 dark:text-[#94a3b8] uppercase">Website URL (optional)</span>
            <input
              {...register("websiteUrl")}
              className="rounded-xl border border-black/10 dark:border-white/10 bg-slate-50 dark:bg-[#020617] px-4 py-3 text-[14px] text-slate-900 dark:text-white outline-none transition-all focus:border-[#22c55e] focus:ring-1 focus:ring-[#22c55e]"
              placeholder="https://..."
        ...
        <div className="flex flex-col gap-3">
          <label className="flex flex-col gap-1">
            <span className="text-[11px] font-bold tracking-wide text-slate-500 dark:text-[#94a3b8] uppercase">Logo URL</span>
            <input
              {...register("logoUrl")}
              className="rounded-xl border border-black/10 dark:border-white/10 bg-slate-50 dark:bg-[#020617] px-4 py-3 text-[14px] text-slate-900 dark:text-white outline-none transition-all focus:border-[#22c55e] focus:ring-1 focus:ring-[#22c55e]"
          ...
          <label className="flex flex-col gap-2 rounded-xl border border-black/10 dark:border-white/10 bg-slate-50 dark:bg-[#020617] p-4 text-[14px]">
            <span className="text-[11px] font-bold tracking-wide text-slate-500 dark:text-[#94a3b8] uppercase">Upload logo (optional)</span>
            <input
              type="file"
              accept="image/*"
              onChange={(e) => {
              ...
              disabled={uploading}
            />
            {logoUrl ? <img src={logoUrl} alt="Client logo preview" className="h-10 w-10 object-contain" /> : null}
          </label>
          ...
          <label className="flex flex-col gap-1">
            ...
            <textarea
              ...
              placeholder="Short description shown in the showcase."
            />
            {errors.description ? (
      ...
      <div className="overflow-auto rounded-2xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#0f172a]">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {clients.map((c) => (
              <tr key={c._id} className="border-b border-black/5 dark:border-white/5">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    {c.logoUrl ? <img src={c.logoUrl} alt="" className="h-8 w-8 object-contain" /> : null}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await fetch(`${API_BASE_URL}/api/admin/clients/${c._id}`, {
                            method: "DELETE",
                            credentials: "include",
                          });
                          await globalMutate(swrKey);
                          setNotification("Client deleted.");
                        } catch (e: any) {
            ...
            {clients.length === 0 ? (
              <tr>
                <td className="px-4 py-6 text-sm text-[#94a3b8]" colSpan={3}>
                  No clients found.
                </td>
              </tr>
            ) : null}
